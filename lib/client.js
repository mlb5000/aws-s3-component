...
class Client {
  constructor(logger, credentials) {
    ...
    const { accessKeyId, secretAccessKey, region } = inputCreds;
    this.s3 = new aws.S3({ accessKeyId, secretAccessKey, region });
    this.sqs = new aws.SQS({ accessKeyId, secretAccessKey, region });
  }
  ...
  async sendSqsMessage(queue, message) {
    const queueUrl = await this.sqs.getQueueUrl({ QueueName: queue }).promise();
